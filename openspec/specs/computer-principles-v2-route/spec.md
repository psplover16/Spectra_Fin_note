...
- **THEN** the first eight route-visible topics are the approved non-catalog topics `加強練習`, `阿姆達爾定律`, `CPU 排班演算法`, `死結`, `分頁與分段記憶體管理`, `物件導向特性`, `複雜度與線性結構`, and `樹與雜湊表`
- **AND** the catalog-backed topic segment starts at the ninth route-visible topic
...
| 3 | `CPU 排班演算法` | `_private/計概補充/CPU排班演算法_國考完整講義.md` |
| 4 | `死結` | `_private/計概補充/死結_考試精簡版.md` |
| 5 | `分頁與分段記憶體管理` | `_private/計概補充/分頁與分段記憶體管理_題目帶動教學完整版.md` |
| 6 | `物件導向特性` | `_private/計概補充/物件導向特性_國考完整講義.md` |
| 7 | `複雜度與線性結構` | `_private/計概補充/基礎資料結構(上)_複雜度與線性結構.md` |
| 8 | `樹與雜湊表` | `_private/計概補充/基礎資料結構(下)_樹與雜湊表.md` |
| 9 | `架構與計算理論` | Catalog row `01` |
