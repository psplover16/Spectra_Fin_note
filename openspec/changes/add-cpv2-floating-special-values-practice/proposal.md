...
- 在 `computer-principles-v2` route 最上方新增獨立「加強練習」section，使用與 route topic 卡片一致的外層容器呈現，直接收錄使用者提供的 12 題提示與資管題目備註。
...
- 補強測試，確認 section 位置、卡片式容器、關鍵短語、sourceFiles 與題庫欄位隔離不被破壞。

## Non-Goals

- 不新增 `/computer-principles-v2` 路由、不改 subject key、catalog order 或 topic 數量。
...
- computer-principles-v2-route: The route SHALL expose a route-level practice section and the floating-point conversion topic SHALL expose an independent IEEE 754 special-value section without changing route ownership or topic order.
- professional-topic-content: Professional topic source traceability SHALL include the added source files for the refreshed floating-point lesson content.
