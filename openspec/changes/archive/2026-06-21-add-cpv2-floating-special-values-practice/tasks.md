...
- [x] 4.4 修正 `Floating point topic exposes practice and IEEE 754 special-value sections` 與 `Refreshed practice and floating point topics trace supplemental sources` 的加強練習 topic 架構契約：`computer-principles-v2` route SHALL 將 `加強練習` 實作為 `未完成` 清單中的第一張正式 `SubjectTopicCard`，id 為 `cpv2-supplemental-practice`，並移除 `SubjectTopicPage` 的 `routeSections` 旁路；完成以 `tests/component/SubjectRoutesSmoke.spec.ts` 驗證卡片位於 `未完成` 內且可展開內容，並以 `tests/unit/computerPrinciplesV2RouteWorkflow.spec.ts`、`tests/unit/subjectTopics.spec.ts`、`tests/unit/professionalTopics.spec.ts` 驗證 topic 順序、sourceFiles 與浮點數特殊值 section 不變。
