...
- **THEN** the route-visible list contains exactly 28 topics
...
- **AND** the eighth topic is `cpv2-supplemental-basic-tree` titled `基礎樹`
- **AND** the ninth topic is `cpv2-supplemental-expression-notation` titled `運算式表示法`
- **AND** the tenth topic is `cpv2-supplemental-avl-tree` titled `AVL樹`
- **AND** the eleventh topic is `cpv2-supplemental-avl-tree-deletion` titled `AVL樹_刪除`
- **AND** the twelfth topic is `cpv2-supplemental-red-black-tree` titled `紅黑樹`
- **AND** the thirteenth topic is `cpv2-supplemental-red-black-tree-deletion` titled `紅黑樹_刪除`
- **AND** the fourteenth topic is `cpv2-supplemental-hash-table` titled `雜湊表`
- **AND** the fifteenth topic is `cpv2-supplemental-trees-hash-practice` titled `樹與雜湊表_考題練習`
- **AND** the sixteenth topic is `cpv2-architecture-computation-theory` titled `架構與計算理論`
...
- **AND** the route-visible list does not contain `cpv2-supplemental-data`, `cpv2-supplemental-basic-data-structures`, `cpv2-supplemental-trees-hash-tables`, `補充資料`, `基礎資料結構`, or `樹與雜湊表`
...
| 8 | `cpv2-supplemental-basic-tree` | `基礎樹` | linked HTML |
| 9 | `cpv2-supplemental-expression-notation` | `運算式表示法` | linked HTML |
| 10 | `cpv2-supplemental-avl-tree` | `AVL樹` | linked HTML |
| 11 | `cpv2-supplemental-avl-tree-deletion` | `AVL樹_刪除` | linked HTML |
| 12 | `cpv2-supplemental-red-black-tree` | `紅黑樹` | linked HTML |
| 13 | `cpv2-supplemental-red-black-tree-deletion` | `紅黑樹_刪除` | linked HTML |
| 14 | `cpv2-supplemental-hash-table` | `雜湊表` | linked HTML |
...
- **THEN** the first fifteen route-visible topics are the approved non-catalog topics `加強練習`, `阿姆達爾定律`, `CPU 排班演算法`, `死結`, `分頁與分段記憶體管理`, `物件導向特性`, `複雜度與線性結構`, `基礎樹`, `運算式表示法`, `AVL樹`, `AVL樹_刪除`, `紅黑樹`, `紅黑樹_刪除`, `雜湊表`, and `樹與雜湊表_考題練習`
- **AND** the catalog-backed topic segment starts at the sixteenth route-visible topic
...
| ----- | ----- | ----- |
| 8 | `基礎樹` | `基礎資料結構(下)_樹與雜湊表.html` 六、樹 |
| 9 | `運算式表示法` | `基礎資料結構(下)_樹與雜湊表.html` 七、運算式 |
| 10 | `AVL樹` | `_private/20260708/AVL樹_考前速記卡.md` |
| 11 | `AVL樹_刪除` | `_private/20260708/AVL樹_刪除專練.md` |
| 12 | `紅黑樹` | `_private/20260708/紅黑樹_考前速記卡.md` |
| 13 | `紅黑樹_刪除` | `_private/20260708/紅黑樹_刪除專練.md` |
| 14 | `雜湊表` | `基礎資料結構(下)_樹與雜湊表.html` 八、雜湊表 |
| 15 | `樹與雜湊表_考題練習` | `基礎資料結構(下)_樹與雜湊表.html` 九～十一 |
| 16 | `架構與計算理論` | Catalog row `01` |
...
- **WHEN** the `computerPrinciplesV2` topic list is loaded
- **THEN** the twenty-fifth route-visible topic has id `cpv2-floating-point-conversion`
...
- **THEN** the route-visible list contains exactly 28 topics
- **AND** the first topic is `cpv2-supplemental-practice` titled `加強練習`
- **AND** the next fourteen topics are `cpv2-supplemental-amdahl-law`, `cpv2-supplemental-cpu-scheduling`, `cpv2-supplemental-deadlock`, `cpv2-supplemental-paging-segmentation`, `cpv2-supplemental-oop-characteristics`, `cpv2-supplemental-complexity-linear-structures`, `cpv2-supplemental-basic-tree`, `cpv2-supplemental-expression-notation`, `cpv2-supplemental-avl-tree`, `cpv2-supplemental-avl-tree-deletion`, `cpv2-supplemental-red-black-tree`, `cpv2-supplemental-red-black-tree-deletion`, `cpv2-supplemental-hash-table`, and `cpv2-supplemental-trees-hash-practice`
- **AND** the sixteenth topic remains `cpv2-architecture-computation-theory` titled `架構與計算理論`
...
- **AND** no route-visible topic has id `cpv2-supplemental-trees-hash-tables` or title `樹與雜湊表`
- **AND** the supplemental topics include content for Amdahl's Law, CPU scheduling, deadlock, paging and segmentation, object-oriented characteristics, complexity and linear data structures, basic tree, expression notation, AVL tree, AVL tree deletion, red-black tree, red-black tree deletion, hash table, and tree and hash-table practice
