...
- [x] 5.3 為計概每個 manifest topic 建立內容生成副代理 task，要求輸出 `_TMP/<timestamp>-computer-principles-<topic>.md` 並採 `lessonArticle` 草稿形狀，包含 sourceFiles、sourceSection、sourceLabels、lead、sections 與 verifier result；以 tasks.md review 驗證 task 數量等於計概 topic 數。
...
- [x] 13.1 將計概 verified 草稿分批匯入正式 app data，讓 `/computer-principles` 使用 `lessonArticle` 正式 topic 取代對應 placeholder 與舊固定模板內容；以頁面 review 驗證已匯入 topic 顯示來源與教材式文章。
...
- [x] 15.2 落實設計決策「計概品質補救採全量 manifest 匯入與草稿品質門檻」，補齊 `_TMP/manifests/computer-principles-manifest.md` 全部 topic 的實質 AI 生成草稿，讓每個計概 topic 都有 verified Markdown draft，且採 `lessonArticle` 形狀，包含來源對應、教材本文、學習標記說明與 verifier 結果，並移除舊固定模板章節；以 `tests/unit/tmpDraftQuality.spec.ts` 驗證草稿章節與狀態。
...
- [x] 15.6 執行計概內容品質驗收，確認每個新增計概 topic 至少有來源、`lessonArticle` 教材正文、來源標記段落、專有名詞中英對照與非空 blocks，且正式 app 不引用 `_TMP` runtime 檔案或 raw import `_private/計算機概論.txt`；以 `npx vitest run tests/unit/professionalTopics.spec.ts tests/unit/tmpDraftQuality.spec.ts tests/unit/contentProductionReports.spec.ts` 驗證。
...
- [x] 16.1 落實設計決策「每個路由單獨執行完整教材產製流程」並驗證 Source label discovery is exhaustive before route drafting；先掃描所有允許來源 txt/md 與本輪輔助來源的方括號候選，將候選分類為 valid source label、auxiliary label、non-label syntax/code token、unknown label，並把所有有效標記寫入 `_private/TMP/source-label-definitions.md`；目前已掃描到的有效種子標記至少包含 `[必背]`、`[比較]`、`[會算]`、`[會畫]`、`[補充]`、`[易混淆]`、`[考點]`、`[建議]`、`[原文提醒]`、`[補充建議]`、`[會做]`、`[會寫]`、`[必練]`、`[會寫虛擬碼]`、`[原文考點]`，輔助來源候選包含 `[原文保留]`；以 PowerShell 取得 `_private` 根目錄允許 txt/md 檔清單後執行 `rg -n -o` 的掃描報告、`_private/TMP/source-label-definitions.md`、route 追蹤表與內容審查驗證沒有未知有效標記未定義，且 `[i]`、`[mid]`、`[1, 2, 3]`、`[P]` 等非標記未被誤收。
- [x] 16.2 實作 Route-scoped content production workflow 的 route 追蹤表模板，讓每個 route 追蹤表都記錄 source file、source section、topic id、source labels、prompt path、draft path、verified path、import target、status、verifier result、manual review result；以 `_private/TMP/<route>/待生成主題清單_<timestamp>.md` 樣張審查驗證欄位完整。
- [x] 16.3 執行 `/computer-principles` 獨立流程，從 `_private/計算機概論.txt` 掃描 topic 與標記，為每個 topic 建立單一 prompt、`.draft.md`、`.verified.md`，再由主流程匯入 `computerPrinciples` formal app data；以 route 追蹤表、`tests/unit/professionalTopics.spec.ts`、`tests/unit/tmpDraftQuality.spec.ts` 與至少三筆人工抽查紀錄驗證。
- [x] 16.4 執行 `/networking` 獨立流程，從 `_private/網概.txt` 掃描 topic 與標記，為每個 topic 建立單一 prompt、`.draft.md`、`.verified.md`，再由主流程匯入 `networking` formal app data；以 route 追蹤表、networking topic 資料檢查、port/protocol 事實審查與至少三筆人工抽查紀錄驗證。
- [x] 16.5 執行 `/database` 獨立流程，從 `_private/資料庫.txt` 掃描 topic 與標記，為每個 topic 建立單一 prompt、`.draft.md`、`.verified.md`，再由主流程匯入 `database` formal app data；以 route 追蹤表、資料庫正規化/SQL/交易概念審查與至少三筆人工抽查紀錄驗證。
- [x] 16.6 執行 `/information-management` 獨立流程，從 `_private/資訊管理.txt` 掃描 topic 與標記，為每個 topic 建立單一 prompt、`.draft.md`、`.verified.md`，再由主流程匯入 `informationManagement` formal app data；以 route 追蹤表、管理名詞與資訊系統流程審查、至少三筆人工抽查紀錄驗證。
- [x] 16.7 執行 `/programming` 獨立流程，從 `_private/程式.txt` 與 `_private/系統分析與設計.txt` 掃描 topic 與標記，為每個 topic 建立單一 prompt、`.draft.md`、`.verified.md`，再由主流程匯入 `programming` formal app data；以 route 追蹤表、Java 範例註解審查、系統分析名詞審查與至少三筆人工抽查紀錄驗證。
- [x] 16.8 執行 `/algorithms` 獨立流程，從 `_private/資料結構與演算法.txt` 與 `_private/國考常見演算法_Java遞迴非遞迴_時間複雜度.md` 掃描 topic 與標記，為每個 topic 建立單一 prompt、`.draft.md`、`.verified.md`，再由主流程匯入 `algorithms` formal app data；以 route 追蹤表、Java 遞迴/非遞迴範例、複雜度、穩定性與至少三筆人工抽查紀錄驗證。
- [x] 16.9 驗證每個 topic 的 Source labels drive teaching draft structure，確認所有 discovered valid source labels 都依 `_private/TMP/source-label-definitions.md` 展開；其中 `[必背]` 產出定義/重要性/最小背誦句/國考作答模板/易錯提醒，`[比較]` 產出比較表或條列比較，`[會算]` 產出公式與代入例題，`[會畫]` 產出圖或文字圖，`[會做]`、`[會寫]`、`[必練]`、`[會寫虛擬碼]` 產出可操作、可練習或可撰寫的步驟、範例與檢核點，`[原文考點]`、`[原文提醒]`、`[原文保留]` 保留來源用意並改寫成教材內容；以 route 追蹤表抽樣、source-label scan report 與內容審查報告驗證。
- [x] 16.10 驗證 Verified draft is the only import source，確認所有 imported topic 都來自 `.verified.md`，且 `.draft.md` 或 blocked topic 不會匯入正式 app data；以 `npx vitest run tests/unit/professionalTopics.spec.ts tests/unit/tmpDraftQuality.spec.ts tests/unit/contentProductionReports.spec.ts`、`npm run build`、`spectra analyze ingest-professional-subject-content --json` 與 `spectra validate ingest-professional-subject-content` 驗證。
- [x] 16.11 落實設計決策「副代理優先承擔 route 內產製與審查」並驗證 Subagents isolate route production work from main integration，確認 route source inventory、topic prompt、content writer、verifier、route integration auditor、import readiness 六類副代理輸出都只落在 `_private/TMP/<route>/`，且 formal app data 僅由主流程在 import readiness 通過後修改；以 route 追蹤表、副代理 final report、import readiness report、`git diff -- src tests PROJECT_ARCHITECTURE.md` 與 content review report 驗證。
- [x] 16.12 落實設計決策「全路由重做時舊教材內容作廢重建」並驗證 Stale professional content is removed during route rebuild，確認第 16 組重做後六個專業 route 的 formal app data 不保留任何沒有本輪 route tracking row、`.verified.md`、verifier result、manual review state 與 import readiness ready 的舊 topic；以 stale topic audit report、`tests/unit/professionalTopics.spec.ts`、`tests/unit/contentProductionReports.spec.ts`、localStorage stale bookmark fixture 與 `/computer-principles`、`/networking`、`/database`、`/information-management`、`/programming`、`/algorithms` 人工抽查驗證。
- [x] 16.13 落實 Computer principles rebuilt topics use lessonArticle content，確認 `/computer-principles` 本輪試點的正式 blocks 僅保留 `lessonArticle`，移除舊固定模板 blocks，並確認 `professionalTopics.ts` 不 raw import `_private/計算機概論.txt`；以 `tests/unit/professionalTopics.spec.ts`、`tests/unit/computerPrinciplesRouteWorkflow.spec.ts`、`tests/unit/tmpDraftQuality.spec.ts` 與 `npm run typecheck` 驗證。
- [x] 16.14 修正計概馮紐曼 pilot 的副代理 prompt 與匯入契約，要求 prompt 保留來源大綱並明確標示它是 writer input，不是成品；內容生成副代理需把 `[必背] 兩大特色`、`[必背] 五大單元`、`[比較] 馮紐曼架構 vs 哈佛架構`、`[必背] 馮紐曼瓶頸` 擴寫成完整新手國考教材，包含定義、中英專有名詞、核心、如何應用、國考寫法、比較表、考場辨認法與最小背誦句；主流程再由 verified draft 匯入正式 `lessonArticle`，並以 `tests/unit/computerPrinciplesRouteWorkflow.spec.ts`、`tests/unit/tmpDraftQuality.spec.ts`、`tests/unit/professionalTopics.spec.ts` 與 `npm run typecheck` 驗證。

## 17. 全路由全部 section 來源大綱教材重生

- [x] 17.1 落實 All rebuilt professional topics use source-outline teaching articles，讓 `/computer-principles`、`/networking`、`/database`、`/information-management`、`/programming`、`/algorithms` 的每個來源 topic/section 都有單一 source-outline prompt，且 prompt 明確宣告來源大綱是 writer input 不是成品；以六個 route 的 `_private/TMP/<route>/待生成主題清單_<timestamp>.md` 與 prompt review 驗證每列都有 prompt path、source outline、source labels 與 writer instruction。
- [x] 17.2 驗證 Professional topics separate exam outline, memory, and understanding content 的新語意，讓 `examOutline`、`memoryPoints`、`understandingNotes` 只作為 metadata 或 prompt input，不再作為全路由重做後的 displayed fixed blocks；以 `tests/unit/professionalTopics.spec.ts` 檢查六個專業 route 的 rebuilt topics 不含舊固定 displayed blocks。
- [x] 17.3 驗證 Imported drafts are substantive AI-generated teaching drafts 擴大到全部專業 route，讓每個 imported topic 都能追到 `_private/TMP/<route>/<topic-id>.draft.md` 與 `.verified.md`，且 draft 包含來源對應、來源大綱 input、教材本文、學習標記說明與 verifier 結果；以 `tests/unit/tmpDraftQuality.spec.ts` 與 route tracking row count 驗證。
- [x] 17.4 依 Source label discovery is exhaustive before route drafting 重新掃描全部允許來源 txt/md 的方括號標記，補齊 `_private/TMP/source-label-definitions.md` 的定義、展開規則與 verifier 檢查點；以 bracket scan report 驗證沒有 unknown label 被直接 verified。
- [x] 17.5 依 Subagents isolate route production work from main integration 派發 route source inventory、topic prompt、content writer、verifier、route integration auditor、import readiness 六類副代理，讓副代理只寫入 `_private/TMP/<route>/` artifact，不修改 `src/`、`tests/` 或 `PROJECT_ARCHITECTURE.md`；以副代理 final report 與 `git diff -- src tests PROJECT_ARCHITECTURE.md` 在副代理階段無變更驗證。
- [x] 17.6 讓 Content writer 副代理針對每個 topic/section 產生教材 draft，內容需展開定義、中英專有名詞、核心概念、如何應用、國考寫法、易錯點、最小背誦句，並依 `[比較]`、`[會算]`、`[會畫]` 等標記補比較表、代入例題或文字圖；以每個 route 至少三筆人工抽查與 draft quality test 驗證。
- [x] 17.7 讓 Verifier 副代理針對每個 draft 輸出 `.verified.md` 或 blocked report，並檢查來源對應、標記展開、事實正確、中英術語、公式/圖/範例、新手可讀性與舊固定模板移除；以 verifier summary、blocked list 與 `tests/unit/tmpDraftQuality.spec.ts` 驗證。
- [x] 17.8 依 Stale professional content is removed during route rebuild 先排除六個專業 route 既有正式教材內容，再只匯入 import readiness ready 的 `.verified.md`，讓 blocked 或缺少 verified 的 topic 不以舊正文 fallback；以 stale topic audit、localStorage stale bookmark fixture 與 `tests/unit/professionalTopics.spec.ts` 驗證。
- [x] 17.9 由主流程重建 formal app data，讓六個專業 route 的 displayed blocks 全部來自本輪 source-outline verified teaching article，舊 section 正文、舊 sourceSummary、舊 verifierSummary 與舊 import summary 不留作正式內容；以 `/computer-principles`、`/networking`、`/database`、`/information-management`、`/programming`、`/algorithms` 人工抽查與 `npm run typecheck` 驗證。
- [x] 17.10 更新內容產製報告與架構文件，讓 `_private/TMP/<route>/import-readiness.md`、content review report 與 `PROJECT_ARCHITECTURE.md` 記錄全路由全部 section 的 source-outline prompt、draft、verified、import readiness 與 obsolete cleanup 流程；以文件 review 驗證與 design 的 Implementation Contract 一致。
- [x] 17.11 執行全量驗收，確認新增規格、任務與實作一致，且六個專業 route 無舊模板 fallback、無 stale topic、無未 verified 匯入；以 `npx vitest run`、`npm run build`、`spectra analyze ingest-professional-subject-content --json` 與 `spectra validate ingest-professional-subject-content` 驗證。
