...
- 內容重做：上述來源大綱 prompt、副代理 draft、verifier、verified-only import 與 obsolete cleanup 流程必須套用到全部專業路由的全部 topic/section；舊固定 section blocks 與低品質正文不得在任何專業 route 中保留為正式內容。
...
- professional-topic-content: 補充 all-section source-outline rebuild，要求全部專業 route 的全部 topic/section 都以來源大綱作為 writer input 重新生成教材，並刪除舊固定 section blocks 與舊正文。
