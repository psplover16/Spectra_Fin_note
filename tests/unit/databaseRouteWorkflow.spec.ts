...
const readDatabaseManifestRows = () =>
  readText('_TMP/manifests/database-manifest.md')
    .split(/\r?\n/)
...
const routeTrackingPath = `${routeRoot}/待生成主題清單_20260613-113000.md`;

describe('database route-scoped content workflow', () => {
  it('creates prompt, draft, and verified files for every database manifest topic', () => {
    ...
    for (const row of manifestRows) {
      ...
      expect(prompt).toContain('_private/資料庫.txt');
  ...
  it('tracks database normalization, SQL, and transaction review through readiness', () => {
    const manifestRows = readDatabaseManifestRows();
    ...
    const conceptReview = readText(`${routeRoot}/database-concept-review.md`);

    const trackingRows = tracking
      .split(/\r?\n/)
      .filter((line) => line.startsWith('| `_private/資料庫.txt` |'));
    ...
    for (const expectedReviewTopic of ['database-normalization', 'database-sql-crud', 'database-acid-transactions']) {
      expect(manualReview).toContain(expectedReviewTopic);
      expect(manualReview).toContain('pass');
    }

    for (const expectedConcept of ['1NF', '2NF', '3NF', 'BCNF', 'ACID', 'Atomicity', 'SQL Injection']) {
      expect(conceptReview).toContain(expectedConcept);
    }
  ...
  it('imports formal database app data for every manifest topic', () => {
    const manifestRows = readDatabaseManifestRows();
    ...
    for (const row of manifestRows) {
      ...
      expect(topic?.sourceFiles).toEqual(expect.arrayContaining(['_private/資料庫.txt']));
