...
- source summary: 整理Register（暫存器）在基本計概中的國考定位，重點包含PC、IR、MAR、MDR。
...
- 暫存器(Register)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 程式計數器(Program Counter)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 指令暫存器(Instruction Register)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 記憶體位址暫存器(Memory Address Register)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 記憶體資料暫存器(Memory Data Register)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
...
題目：判斷題目問到「Register（暫存器）」時，如何快速定位考點？

1. 先把題目中的中文關鍵字對到 Register(Register)。
2. 再看題目要求的是定義、比較、計算還是流程。
3. 最後用來源提醒「PC、IR、MAR、MDR」檢查答案是否涵蓋常考陷阱。

結果：能把 Register(Register) 放回 3a. 基本計概 / Register（暫存器） 的脈絡，就是本 topic 的最低通過線。
