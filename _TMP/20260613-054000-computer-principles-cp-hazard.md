...
- source summary: 整理Hazard在基本計概中的國考定位，重點包含structural/data/control。
...
- 管線危障(Pipeline Hazard)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 結構危障(Structural Hazard)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 資料危障(Data Hazard)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 控制危障(Control Hazard)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 資料前遞(Forwarding)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
...
題目：判斷題目問到「Hazard」時，如何快速定位考點？

1. 先把題目中的中文關鍵字對到 Hazard(Pipeline Hazard)。
2. 再看題目要求的是定義、比較、計算還是流程。
3. 最後用來源提醒「structural/data/control」檢查答案是否涵蓋常考陷阱。

結果：能把 Hazard(Pipeline Hazard) 放回 3a. 基本計概 / Hazard 的脈絡，就是本 topic 的最低通過線。
