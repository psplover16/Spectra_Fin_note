---
topic_id: cp-deadlock
subject: computerPrinciples
source_files:
  - _private/計算機概論.txt
status: verified
generated_at: "2026-06-13T05:40:00+08:00"
verified_by: content-verifier
---

# Deadlock(Deadlock)

## 來源對應

- source file: `_private/計算機概論.txt`
- source section: `3c. 作業系統 / Deadlock`
- source summary: 整理Deadlock在作業系統中的國考定位，重點包含四必要條件與 Banker。

## 國考重點

- 能說明Deadlock(Deadlock) 的定義、用途與常考問法。
- 能把來源段落「3c. 作業系統 / Deadlock」整理成考試大綱、記憶重點與理解說明。
- 能用實際例子或操作步驟驗證 四必要條件與 Banker。

## 國考速記

- Deadlock 的速記核心是：四必要條件與 Banker。
- 看到 Deadlock 先判斷題型是定義、比較、流程還是計算。
- 作答時先寫中文術語，再補英文原名，符合專有名詞中英並列規則。

## 名詞解釋

- 死結(Deadlock)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 互斥(Mutual Exclusion)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 持有並等待(Hold and Wait)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 不可搶奪(No Preemption)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 循環等待(Circular Wait)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 銀行家演算法(Banker's Algorithm)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。

## 核心想法

- Deadlock 不是孤立名詞，而是 作業系統 題組中用來判斷概念、流程或計算的節點。
- 初學者先問「這東西解決什麼問題」，再背公式、表格或比較欄位。
- 若題目出現相近名詞，先用來源提醒「四必要條件與 Banker」排除錯誤選項。

## 實際例子

題目：判斷題目問到「Deadlock」時，如何快速定位考點？

1. 先把題目中的中文關鍵字對到 Deadlock(Deadlock)。
2. 再看題目要求的是定義、比較、計算還是流程。
3. 最後用來源提醒「四必要條件與 Banker」檢查答案是否涵蓋常考陷阱。

結果：能把 Deadlock(Deadlock) 放回 3c. 作業系統 / Deadlock 的脈絡，就是本 topic 的最低通過線。

## 易錯提醒

- 四個必要條件缺一就不會形成 deadlock。
- Avoidance 是事前避免不安全狀態，不是事後偵測。

## 專有名詞

- 死結(Deadlock)
- 互斥(Mutual Exclusion)
- 持有並等待(Hold and Wait)
- 不可搶奪(No Preemption)
- 循環等待(Circular Wait)
- 銀行家演算法(Banker's Algorithm)

## Verifier 結果

- source mapping: verified
- exam outline: verified
- memory points: verified
- beginner explanation: verified
- concrete example or procedure: verified
- bilingual terminology: verified
- issue list: none
- fix summary: 依使用者品質回饋補為實質 AI 教學草稿，不再以 manifest、任務表或摘要充當內容。
- final_status: verified
