---
topic_id: cp-hardware-protection
subject: computerPrinciples
source_files:
  - _private/計算機概論.txt
status: verified
generated_at: "2026-06-13T05:40:00+08:00"
verified_by: content-verifier
---

# 硬體保護(Hardware Protection)

## 來源對應

- source file: `_private/計算機概論.txt`
- source section: `3c. 作業系統 / 硬體保護`
- source summary: 整理硬體保護在作業系統中的國考定位，重點包含dual mode。

## 國考重點

- 能說明硬體保護(Hardware Protection) 的定義、用途與常考問法。
- 能把來源段落「3c. 作業系統 / 硬體保護」整理成考試大綱、記憶重點與理解說明。
- 能用實際例子或操作步驟驗證 dual mode。

## 國考速記

- 硬體保護 的速記核心是：dual mode。
- 看到 Hardware Protection 先判斷題型是定義、比較、流程還是計算。
- 作答時先寫中文術語，再補英文原名，符合專有名詞中英並列規則。

## 名詞解釋

- 硬體保護(Hardware Protection)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 特權指令(Privileged Instruction)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 使用者模式(User Mode)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 核心模式(Kernel Mode)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。
- 計時器中斷(Timer Interrupt)：說明本 topic 的核心術語；作答時先用中文定義，再補英文名稱與使用情境。

## 核心想法

- 硬體保護 不是孤立名詞，而是 作業系統 題組中用來判斷概念、流程或計算的節點。
- 初學者先問「這東西解決什麼問題」，再背公式、表格或比較欄位。
- 若題目出現相近名詞，先用來源提醒「dual mode」排除錯誤選項。

## 實際例子

題目：判斷題目問到「硬體保護」時，如何快速定位考點？

1. 先把題目中的中文關鍵字對到 硬體保護(Hardware Protection)。
2. 再看題目要求的是定義、比較、計算還是流程。
3. 最後用來源提醒「dual mode」檢查答案是否涵蓋常考陷阱。

結果：能把 硬體保護(Hardware Protection) 放回 3c. 作業系統 / 硬體保護 的脈絡，就是本 topic 的最低通過線。

## 易錯提醒

- 不要只背 硬體保護 的標題，至少要能說出定義、用途與一個例子。
- 看到英文 Hardware Protection 時，要能回到中文 硬體保護，避免術語對不上。
- 若題目要求計算或流程，答案要有步驟，不可只寫結論。

## 專有名詞

- 硬體保護(Hardware Protection)
- 特權指令(Privileged Instruction)
- 使用者模式(User Mode)
- 核心模式(Kernel Mode)
- 計時器中斷(Timer Interrupt)

## Verifier 結果

- source mapping: verified
- exam outline: verified
- memory points: verified
- beginner explanation: verified
- concrete example or procedure: verified
- bilingual terminology: verified
- issue list: none
- fix summary: 依使用者品質回饋補為實質 AI 教學草稿，不再以 manifest、任務表或摘要充當內容。
- final_status: verified
