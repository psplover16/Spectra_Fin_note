---
topic_id: programming-translator-comparison
formal_topic_id: programming-programming-translator-comparison
subject: programming
source_files:
  - _private/程式.txt
status: draft
...
# Assembler、Compiler、Interpreter 比較(Assembler Compiler and Interpreter Comparison)

## 來源對應

- source files: `_private/程式.txt`
- source section: `_private/程式.txt` L15-L18`
- source labels: [比較]
...
- source section: `_private/程式.txt` L15-L18`
- topic id: `programming-translator-comparison`
- route: `/programming`
...
### [比較] 考試重點展開
- 能說明 Assembler、Compiler、Interpreter 比較(Assembler Compiler and Interpreter Comparison) 的定義、用途與國考常見問法。
- 能從來源段落「_private/程式.txt` L15-L18」整理出記憶點、理解說明、例子與易錯提醒。
- 能把程式語言名詞、Java 作答思路或語法觀念用中文(English Term) 格式表達。

### [必背] 最小背誦句與記憶支架
- 組譯器(Assembler) 是本 topic 的第一個必背名詞。
- 編譯器(Compiler) 要和 組譯器 一起放入同一個比較或流程脈絡。
- 程式題要能寫出語法意義、執行結果或 Java/C/Python 的語言差異。

### [必背] 名詞定義與雙語術語
- 組譯器(Assembler)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。
- 編譯器(Compiler)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。
- 直譯器(Interpreter)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。
- 機器碼(Machine Code)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。

### [必背] 核心理解與應用脈絡
- 程式類題目不是只背關鍵字，組譯器(Assembler) 要能說明資料怎麼存、流程怎麼走、錯誤怎麼發生。
- 若題目涉及 Java，作答要用註解說明為什麼先判斷條件、為什麼更新變數、為什麼回傳結果。
- 若題目是比較題，答案要列出差異原因與使用情境，而不是只排英文名詞。

### [會做] 實際例子與操作步驟
題目：如何分辨 Assembler、Compiler、Interpreter？

1. Assembler 把組合語言轉成機器碼，層級最接近硬體。
2. Compiler 先把高階語言整體或大段翻成目標碼，再執行。
3. Interpreter 逐行或逐步解讀執行，常見特色是互動性高但執行成本可能較高。

結果：比較題要寫輸入語言、轉換方式、執行時機與典型優缺點。
...
### [必背] 新手讀法與國考作答線
- 本主題的來源大綱只是輸入，不是最後答案；讀者要先知道「Assembler、Compiler、Interpreter 比較」在考試中通常是在問定義、流程、比較、計算或應用判斷。
