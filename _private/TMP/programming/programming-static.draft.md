---
topic_id: programming-static
formal_topic_id: programming-programming-static
subject: programming
source_files:
  - _private/程式.txt
status: draft
...
# 程式 static 觀念(Static Concept)

## 來源對應

- source files: `_private/程式.txt`
- source section: `_private/程式.txt` L34-L37`
- source labels: [必背], [會寫]
...
### [必背] 考試重點展開
- 能說明 程式 static 觀念(Static Concept) 的定義、用途與國考常見問法。
- 能從來源段落「_private/程式.txt` L34-L37」整理出記憶點、理解說明、例子與易錯提醒。
- 能把程式語言名詞、Java 作答思路或語法觀念用中文(English Term) 格式表達。

### [必背] 最小背誦句與記憶支架
- 靜態變數(Static Variable) 是本 topic 的第一個必背名詞。
- 連結性(Linkage) 要和 靜態變數 一起放入同一個比較或流程脈絡。
- 程式題要能寫出語法意義、執行結果或 Java/C/Python 的語言差異。

### [必背] 名詞定義與雙語術語
- 靜態變數(Static Variable)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。
- 連結性(Linkage)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。
- 類別成員(Class Member)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。

### [必背] 核心理解與應用脈絡
- 程式類題目不是只背關鍵字，靜態變數(Static Variable) 要能說明資料怎麼存、流程怎麼走、錯誤怎麼發生。
- 若題目涉及 Java，作答要用註解說明為什麼先判斷條件、為什麼更新變數、為什麼回傳結果。
- 若題目是比較題，答案要列出差異原因與使用情境，而不是只排英文名詞。

### [會做] 實際例子與操作步驟
題目：如何把「程式 static 觀念」整理成可作答教材？

1. 先定位來源段落：_private/程式.txt` L34-L37。
2. 再寫出 靜態變數(Static Variable) 的定義與用途。
3. 最後補一個判斷線索、比較點或操作步驟，讓答案不只停在名詞背誦。

結果：程式 static 觀念 的最低作答線是定義清楚、英文對得上、能舉例並能指出易錯差異。
...
### [必背] 新手讀法與國考作答線
- 本主題的來源大綱只是輸入，不是最後答案；讀者要先知道「程式 static 觀念」在考試中通常是在問定義、流程、比較、計算或應用判斷。
