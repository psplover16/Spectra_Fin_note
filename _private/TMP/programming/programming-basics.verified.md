...
formal_topic_id: programming-programming-basics
subject: programming
source_files:
  - _private/程式.txt
status: verified
...
# 程式基礎(Programming Basics)

## 來源對應

- source files: `_private/程式.txt`
- source section: `_private/程式.txt` L13-L38`
- source labels: [必背]
...
### [必背] 考試重點展開
- 能說明 程式基礎(Programming Basics) 的定義、用途與國考常見問法。
- 能從來源段落「_private/程式.txt` L13-L38」整理出記憶點、理解說明、例子與易錯提醒。
- 能把程式語言名詞、Java 作答思路或語法觀念用中文(English Term) 格式表達。

### [必背] 最小背誦句與記憶支架
- 變數(Variable) 是本 topic 的第一個必背名詞。
- 函式(Function) 要和 變數 一起放入同一個比較或流程脈絡。
...
- 迴圈(Loop)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。
- 條件判斷(Conditional Statement)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。
...
### [會做] 實際例子與操作步驟
題目：如何把「程式基礎」整理成可作答教材？

1. 先定位來源段落：_private/程式.txt` L13-L38。
2. 再寫出 變數(Variable) 的定義與用途。
3. 最後補一個判斷線索、比較點或操作步驟，讓答案不只停在名詞背誦。

結果：程式基礎 的最低作答線是定義清楚、英文對得上、能舉例並能指出易錯差異。
...
### [必背] 新手讀法與國考作答線
- 本主題的來源大綱只是輸入，不是最後答案；讀者要先知道「程式基礎」在考試中通常是在問定義、流程、比較、計算或應用判斷。
