---
topic_id: security
formal_topic_id: networking-security
subject: networking
source_files:
  - _private/網概.txt
status: draft
...
### [必背] 考試重點展開
- 能說明 資訊安全(Information Security) 的定義與國考常見問法。
- 能從來源段落「十一、資訊安全」整理出記憶點、理解重點與易錯處。
- 能把專有名詞以中文(English Term) 格式寫出，並用例子檢查是否真的理解。

### [必背] 最小背誦句與記憶支架
- 資訊安全 的第一步是先背核心名詞與層級位置。
- 看到 Information Security 時，要能回到來源段落 十一、資訊安全。
- 若題目問比較，先列功能、層級、代表協定或設備，再寫差異原因。

### [必背] 名詞定義與雙語術語
- 機密性(Confidentiality)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 完整性(Integrity)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 可用性(Availability)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 防火牆(Firewall)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 入侵偵測系統(Intrusion Detection System)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 零信任(Zero Trust)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。

### [必背] 核心理解與應用脈絡
- 網概不是單純背名詞，而是把「層級、功能、協定、設備、攻擊、防護」放在同一張心智圖。
- 資訊安全 要先知道它位在哪一層、解決什麼問題、和哪些相近概念容易混淆。
- 考題常把服務名、port、協定、設備或安全目標互相交叉，所以教材要能互相對照。

### [會做] 實際例子與操作步驟
題目：遇到資安事件題，如何從攻擊對應到防護策略？

1. 先判斷攻擊破壞 CIA 的哪一項：Confidentiality、Integrity、Availability。
2. 再判斷攻擊型態，例如 Phishing、SQL Injection、DDoS 或 MITM。
3. 最後對應控制措施，例如 MFA、WAF、IDS/IPS、Backup 3-2-1 或 Least Privilege。

結果：資安題不能只背名詞，要能從事件推回目標、攻擊與防禦。
...
### [必背] 新手讀法與國考作答線
- 本主題的來源大綱只是輸入，不是最後答案；讀者要先知道「資訊安全」在考試中通常是在問定義、流程、比較、計算或應用判斷。
