---
topic_id: osi-tcpip-models
formal_topic_id: networking-osi-tcpip-models
subject: networking
source_files:
  - _private/網概.txt
status: draft
...
# OSI 7 層與 TCP/IP 5 層(OSI Model and TCP/IP Model)
...
### [必背] 考試重點展開
- 能說明 OSI 7 層與 TCP/IP 5 層(OSI Model and TCP/IP Model) 的定義與國考常見問法。
- 能從來源段落「五、OSI 7 層與 TCP/IP 5 層」整理出記憶點、理解重點與易錯處。
- 能把專有名詞以中文(English Term) 格式寫出，並用例子檢查是否真的理解。

### [必背] 最小背誦句與記憶支架
- OSI 7 層與 TCP/IP 5 層 的第一步是先背核心名詞與層級位置。
- 看到 OSI Model and TCP/IP Model 時，要能回到來源段落 五、OSI 7 層與 TCP/IP 5 層。
- 若題目問比較，先列功能、層級、代表協定或設備，再寫差異原因。

### [必背] 名詞定義與雙語術語
- 開放式系統互連模型(Open Systems Interconnection Model)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 傳輸控制協定/網際網路協定(Transmission Control Protocol/Internet Protocol)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 協定資料單元(Protocol Data Unit)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。

### [必背] 核心理解與應用脈絡
- 網概不是單純背名詞，而是把「層級、功能、協定、設備、攻擊、防護」放在同一張心智圖。
- OSI 7 層與 TCP/IP 5 層 要先知道它位在哪一層、解決什麼問題、和哪些相近概念容易混淆。
- 考題常把服務名、port、協定、設備或安全目標互相交叉，所以教材要能互相對照。

### [會做] 實際例子與操作步驟
題目：如何把 OSI 7 層映射到 TCP/IP 5 層？

1. OSI Application、Presentation、Session 合併到 TCP/IP Application。
2. OSI Transport 對應 TCP/IP Transport。
3. OSI Network 對應 TCP/IP Internet。
4. OSI Data Link 與 Physical 分別對應 TCP/IP Data Link 與 Physical。

結果：考試常問上三層合併，以及協定或設備歸屬在哪一層。
...
### [必背] 新手讀法與國考作答線
- 本主題的來源大綱只是輸入，不是最後答案；讀者要先知道「OSI 7 層與 TCP/IP 5 層」在考試中通常是在問定義、流程、比較、計算或應用判斷。
