---
topic_id: bubble-sort
formal_topic_id: algorithms-bubble-sort
subject: algorithms
source_files:
...
- source section: `一、氣泡排序法 Bubble Sort`
...
### [必背] 考試重點展開
- 能說明 氣泡排序法(Bubble Sort) 的核心流程、前提與國考常見問法。
- 能寫出 Java 遞迴或非遞迴版本，並用註解說明作答思路。
- 能列出最佳、平均、最差時間複雜度與穩定性或不適用原因。

### [必背] 最小背誦句與記憶支架
- 有 swapped early stop 時最佳 O(n)，平均與最差 O(n^2)，穩定。
- 程式碼不要死背，要能說明每個迴圈、遞迴或邊界更新的理由。
- 複雜度答案要分清楚最佳、平均與最差情況。

### [必背] 名詞定義與雙語術語
- 氣泡排序法(Bubble Sort)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。

### [必背] 核心理解與應用脈絡
- 氣泡排序法(Bubble Sort) 要先掌握資料如何移動或問題如何縮小，再背 Java 寫法。
...
氣泡排序法(Bubble Sort) 的遞迴教學版，註解說明作答思路與終止條件。

```java
void bubbleSortRecursive(int[] a, int n) { if (n <= 1) return; for (int i = 0; i < n - 1; i++) { if (a[i] > a[i + 1]) { int t = a[i]; a[i] = a[i + 1]; a[i + 1] = t; } } bubbleSortRecursive(a, n - 1); }
```

### 氣泡排序法非遞迴版

氣泡排序法(Bubble Sort) 的非遞迴版，註解說明迴圈狀態與更新理由。

```java
void bubbleSort(int[] a) { for (int end = a.length - 1; end > 0; end--) { boolean swapped = false; for (int i = 0; i < end; i++) { if (a[i] > a[i + 1]) { int t = a[i]; a[i] = a[i + 1]; a[i + 1] = t; swapped = true; } } if (!swapped) break; } }
```
...
### [必背] 新手讀法與國考作答線
- 本主題的來源大綱只是輸入，不是最後答案；讀者要先知道「氣泡排序法」在考試中通常是在問定義、流程、比較、計算或應用判斷。
