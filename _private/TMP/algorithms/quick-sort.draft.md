---
topic_id: quick-sort
formal_topic_id: algorithms-quick-sort
subject: algorithms
source_files:
...
- source section: `二、快速排序法 Quick Sort`
...
### [必背] 考試重點展開
- 能說明 快速排序法(Quick Sort) 的核心流程、前提與國考常見問法。
- 能寫出 Java 遞迴或非遞迴版本，並用註解說明作答思路。
- 能列出最佳、平均、最差時間複雜度與穩定性或不適用原因。

### [必背] 最小背誦句與記憶支架
- 平均 O(n log n)，pivot 選不好會退化 O(n^2)，不穩定。
- 程式碼不要死背，要能說明每個迴圈、遞迴或邊界更新的理由。
- 複雜度答案要分清楚最佳、平均與最差情況。

### [必背] 名詞定義與雙語術語
- 快速排序法(Quick Sort)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。
- 基準值(Pivot)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。
- 分割(Partition)：本 topic 的必要術語；作答時先寫中文，再補英文全名、定義、用途或判斷線索。

### [必背] 核心理解與應用脈絡
- 快速排序法(Quick Sort) 要先掌握資料如何移動或問題如何縮小，再背 Java 寫法。
...
1. 選 pivot。
2. Partition 讓 pivot 左小右大。
3. 遞迴或 stack 處理左右區間。

結果：快速排序法 的流程、Java 寫法與複雜度已對上來源。
...
快速排序法(Quick Sort) 的遞迴教學版，註解說明作答思路與終止條件。

```java
void quickSort(int[] a, int left, int right) { if (left >= right) return; int pivot = partition(a, left, right); quickSort(a, left, pivot - 1); quickSort(a, pivot + 1, right); }
```

### 快速排序法非遞迴版

快速排序法(Quick Sort) 的非遞迴版，註解說明迴圈狀態與更新理由。

```java
void quickSortIterative(int[] a) { Deque<int[]> stack = new ArrayDeque<>(); stack.push(new int[] {0, a.length - 1}); while (!stack.isEmpty()) { int[] r = stack.pop(); if (r[0] >= r[1]) continue; int p = partition(a, r[0], r[1]); stack.push(new int[] {r[0], p - 1}); stack.push(new int[] {p + 1, r[1]}); } }
```
...
### [必背] 新手讀法與國考作答線
- 本主題的來源大綱只是輸入，不是最後答案；讀者要先知道「快速排序法」在考試中通常是在問定義、流程、比較、計算或應用判斷。
