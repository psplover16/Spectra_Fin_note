---
topic_id: database-sql-crud
formal_topic_id: database-database-sql-crud
subject: database
source_files:
  - _private/資料庫.txt
status: draft
...
### [必背] 考試重點展開
- 能說明 SQL 分類與 CRUD(SQL Categories and CRUD) 的定義與國考常見問法。
- 能從來源範圍「_private/資料庫.txt` lines 81-101」整理出記憶重點、理解說明與操作步驟。
- 能把資料庫專有名詞以中文(English Term) 格式寫出。

### [必背] 最小背誦句與記憶支架
- SQL 分類與 CRUD 的第一步是分清楚資料、結構、限制與操作。
- 看到 SQL Categories and CRUD 時，要能回到 key、relation、SQL、transaction 或 data model 的脈絡。
- 若題目要求比較，先列條件，再寫差異原因與適用場景。

### [必背] 名詞定義與雙語術語
- 資料定義語言(Data Definition Language)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 資料操作語言(Data Manipulation Language)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 資料控制語言(Data Control Language)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 交易控制語言(Transaction Control Language)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 建立讀取更新刪除(Create Read Update Delete)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。

### [必背] 核心理解與應用脈絡
- 資料庫題常把設計、語法與交易保證混在同一題，不能只背名詞。
- SQL 分類與 CRUD 要先知道它解決的問題，再判斷是否需要畫圖、寫 SQL、拆表或比較。
- 答題時要避免把 DB、DBMS、Data Model、SQL 與 Transaction 混成同一層。

### [會做] 實際例子與操作步驟
題目：如何把 CRUD 對應到 SQL 指令？

1. Create 對應 INSERT。
2. Read 對應 SELECT。
3. Update 對應 UPDATE。
4. Delete 對應 DELETE。
5. DDL 負責 CREATE TABLE、ALTER TABLE、DROP TABLE；DCL 負責 GRANT、REVOKE。

結果：CRUD 是操作意圖，SQL 類別與語法是落地寫法。
...
- 建立讀取更新刪除(Create Read Update Delete)

### [必背] 新手讀法與國考作答線
- 本主題的來源大綱只是輸入，不是最後答案；讀者要先知道「SQL 分類與 CRUD」在考試中通常是在問定義、流程、比較、計算或應用判斷。
