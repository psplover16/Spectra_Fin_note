---
topic_id: database-nosql
formal_topic_id: database-database-nosql
subject: database
source_files:
  - _private/資料庫.txt
status: draft
...
- source labels: [必背], [比較], [會寫], [必練], [易混淆], [會做]
...
### [必背] 考試重點展開
- 能說明 NoSQL(NoSQL) 的定義與國考常見問法。
- 能從來源範圍「_private/資料庫.txt` lines 116-128」整理出記憶重點、理解說明與操作步驟。
- 能把資料庫專有名詞以中文(English Term) 格式寫出。

### [必背] 最小背誦句與記憶支架
- NoSQL 的第一步是分清楚資料、結構、限制與操作。
- 看到 NoSQL 時，要能回到 key、relation、SQL、transaction 或 data model 的脈絡。
- 若題目要求比較，先列條件，再寫差異原因與適用場景。

### [必背] 名詞定義與雙語術語
- 鍵值資料庫(Key-Value Database)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 文件資料庫(Document Database)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 欄族資料庫(Column-Family Database)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 圖形資料庫(Graph Database)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 一致性可用性分割容忍(CAP)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。

### [必背] 核心理解與應用脈絡
- 資料庫題常把設計、語法與交易保證混在同一題，不能只背名詞。
- NoSQL 要先知道它解決的問題，再判斷是否需要畫圖、寫 SQL、拆表或比較。
- 答題時要避免把 DB、DBMS、Data Model、SQL 與 Transaction 混成同一層。

### [會做] 實際例子與操作步驟
題目：如何把「NoSQL」整理成資料庫考點？

1. 先定位來源範圍：_private/資料庫.txt` lines 116-128。
2. 再把主題拆成定義、比較、範例、易錯點。
3. 若涉及設計或語法，補上可操作步驟。

結果：NoSQL 至少要能說明定義、用途、例子與考試常問陷阱。
...
- 欄族資料庫(Column-Family Database)
- 圖形資料庫(Graph Database)
- 一致性可用性分割容忍(CAP)

### [必背] 新手讀法與國考作答線
- 本主題的來源大綱只是輸入，不是最後答案；讀者要先知道「NoSQL」在考試中通常是在問定義、流程、比較、計算或應用判斷。
