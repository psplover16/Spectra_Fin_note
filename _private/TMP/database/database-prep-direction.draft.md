---
topic_id: database-prep-direction
formal_topic_id: database-database-prep-direction
subject: database
source_files:
  - _private/資料庫.txt
status: draft
...
# 準備方向(Database Preparation Direction)
...
### [必背] 考試重點展開
- 能說明 準備方向(Database Preparation Direction) 的定義與國考常見問法。
- 能從來源範圍「_private/資料庫.txt` lines 3-8」整理出記憶重點、理解說明與操作步驟。
- 能把資料庫專有名詞以中文(English Term) 格式寫出。

### [必背] 最小背誦句與記憶支架
- 準備方向 的第一步是分清楚資料、結構、限制與操作。
- 看到 Database Preparation Direction 時，要能回到 key、relation、SQL、transaction 或 data model 的脈絡。
- 若題目要求比較，先列條件，再寫差異原因與適用場景。

### [必背] 名詞定義與雙語術語
- 結構化查詢語言(Structured Query Language)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 資料庫管理系統(Database Management System)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 方言(Dialect)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。

### [必背] 核心理解與應用脈絡
- 資料庫題常把設計、語法與交易保證混在同一題，不能只背名詞。
- 準備方向 要先知道它解決的問題，再判斷是否需要畫圖、寫 SQL、拆表或比較。
- 答題時要避免把 DB、DBMS、Data Model、SQL 與 Transaction 混成同一層。

### [會做] 實際例子與操作步驟
題目：如何把「準備方向」整理成資料庫考點？

1. 先定位來源範圍：_private/資料庫.txt` lines 3-8。
2. 再把主題拆成定義、比較、範例、易錯點。
3. 若涉及設計或語法，補上可操作步驟。

結果：準備方向 至少要能說明定義、用途、例子與考試常問陷阱。
