---
topic_id: database-keys
formal_topic_id: database-database-keys
subject: database
source_files:
  - _private/資料庫.txt
status: draft
...
### [必背] 考試重點展開
- 能說明 Key(Keys) 的定義與國考常見問法。
- 能從來源範圍「_private/資料庫.txt` lines 45-53」整理出記憶重點、理解說明與操作步驟。
- 能把資料庫專有名詞以中文(English Term) 格式寫出。

### [必背] 最小背誦句與記憶支架
- Key 的第一步是分清楚資料、結構、限制與操作。
- 看到 Keys 時，要能回到 key、relation、SQL、transaction 或 data model 的脈絡。
- 若題目要求比較，先列條件，再寫差異原因與適用場景。

### [必背] 名詞定義與雙語術語
- 超鍵(Super Key)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 候選鍵(Candidate Key)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 主鍵(Primary Key)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 外鍵(Foreign Key)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。
- 複合鍵(Composite Key)：本 topic 的核心術語；作答時先寫中文，再補英文全名與用途。

### [必背] 核心理解與應用脈絡
- 資料庫題常把設計、語法與交易保證混在同一題，不能只背名詞。
- Key 要先知道它解決的問題，再判斷是否需要畫圖、寫 SQL、拆表或比較。
- 答題時要避免把 DB、DBMS、Data Model、SQL 與 Transaction 混成同一層。

### [會做] 實際例子與操作步驟
題目：如何把「Key」整理成資料庫考點？

1. 先定位來源範圍：_private/資料庫.txt` lines 45-53。
2. 再把主題拆成定義、比較、範例、易錯點。
3. 若涉及設計或語法，補上可操作步驟。

結果：Key 至少要能說明定義、用途、例子與考試常問陷阱。
...
### [必背] 新手讀法與國考作答線
- 本主題的來源大綱只是輸入，不是最後答案；讀者要先知道「Key」在考試中通常是在問定義、流程、比較、計算或應用判斷。
