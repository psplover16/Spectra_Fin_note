...
本筆記的「時間複雜度驗證」以最壞情況為主，符合國考常見預設問法。
...
public class AlgorithmExamJava {
    ...
    public static void main(String[] args) {
        // original 是同一組測試資料，包含未排序、重複值與負數，方便檢查排序是否真的正確。
        int[] original = {5, 1, 4, 2, 8, 5, -3};

        // expected 使用 Java 內建排序先做出標準答案，之後每個排序方法都要排成一樣。
        int[] expected = original.clone();
        Arrays.sort(expected);

        // 每個排序方法都用同一組資料測試，避免某個方法只剛好通過特定資料。
    ...
    private static void testSort(String name, int[] original, int[] expected, SortFunction function) {
        // 每次測試都複製一份 original，避免前一個排序方法改到資料後影響下一個方法。
        int[] data = original.clone();

        // 執行傳進來的排序方法。
        function.sort(data);

        // 排序結果必須和 expected 完全相同，否則直接丟錯，指出是哪個方法失敗。
...
| 演算法 | 遞迴版本最壞時間 | 非遞迴版本最壞時間 | 重要備註 |
|---|---|---|---|
| 氣泡排序 Bubble Sort | O(n²) | O(n²) | 相鄰比較，大的往右冒泡 |
| 快速排序 Quick Sort | O(n²) | O(n²) | pivot 若切得不平均會退化 |
| Fibonacci | O(2ⁿ) | O(n) | 遞迴版會大量重複計算 |
| 最大公因數 GCD | O(log min(a,b)) | O(log min(a,b)) | 使用歐幾里得演算法 |
| 二元搜尋 Binary Search | O(log n) | O(log n) | 陣列必須先排序 |
| 選擇排序 Selection Sort | O(n²) | O(n²) | 每輪都要找最小值 |
| 插入排序 Insertion Sort | O(n²) | O(n²) | 反向排序時搬移最多 |
...
```text
氣泡排序：相鄰比較，大的往右冒泡，最壞 O(n²)
快速排序：pivot 分左右，最壞 O(n²)
